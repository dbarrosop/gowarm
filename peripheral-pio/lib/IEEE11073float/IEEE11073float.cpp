...
uint32_t float2IEEE11073(float data, uint8_t output[4]) {
...
float decodeIEEE11073(uint8_t data[4]) {
    uint8_t b0 = data[0];
    uint8_t b1 = data[1];
    uint8_t b2 = data[2];
    int8_t exponent = data[3];

    bool negative = b2 > 127;

    uint8_t b3 = 0;
    if (negative) {
        b3 = 0xff;
    }
    float mantissa = b0 | (b1 << 8) | (b2 << 16) | (b3 << 24);
